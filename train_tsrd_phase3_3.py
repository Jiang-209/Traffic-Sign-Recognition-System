"""
train_tsrd_phase3_3.py — TSRD ROI-focused Preprocessing (Phase 3.3)

实验目的：利用 annotation bbox 裁剪交通标志 ROI 区域（保留 10% 背景边距），
再 resize 到 32×32 进行训练，验证去掉多余背景是否能提升分类效果。

与 Phase 2 基线唯一区别：数据加载使用 load_roi_tsrd_images() 替代 load_tsrd_images()。
其余配置（模型、增强、L2、dropout、LR schedule 等）与 Phase 2 完全一致。

输出:
    tsrd_runs/scratch_phase3_3_YYYYMMDD_HHMMSS/
    ├── logs/
    ├── plots/
    ├── reports/
    └── checkpoints/
...
# ── Argument parser ──
import argparse
parser = argparse.ArgumentParser(description='TSRD Scratch CNN — Phase 3.3 (ROI crop)')
...
if args.run_name:
    RUN_DIR_NAME = f'scratch_phase3_3_{args.run_name}'
else:
    RUN_DIR_NAME = f'scratch_phase3_3_{RUN_ID}'
...
print('=' * 60)
print('  Phase 3.3: TSRD ROI-focused Preprocessing')
print('  58-class scratch CNN — bbox crop + 10% padding')
print('=' * 60)
...
X_train_full, y_train_full = load_roi_tsrd_images(TSRD_TRAIN_DIR, TRAIN_ANNOTATION, expand_ratio=0.1)
X_test, y_test = load_roi_tsrd_images(TSRD_TEST_DIR, TEST_ANNOTATION, expand_ratio=0.1)
